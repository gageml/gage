...
impl Store {
    /// List every blob and tree beneath `reference`, recursively, via
    /// `git ls-tree -r -t -l`. `reference` is a git tree-ish (ref
    /// path, sha, or `<ref>:<path>`).
    pub fn ls(&self, reference: &str) -> Result<Vec<TreeEntry>, StoreError> {
        let out = run(git_in(
            self.path(),
            ["ls-tree", "-r", "-t", "-l", reference],
        ))?;
        parse_ls_tree(&out)
    }
    ...
    pub fn list_tree(&self, tree_or_commit: &str) -> Result<Vec<TreeEntry>, StoreError> {
        let out = run(git_in(self.path(), ["ls-tree", "-l", tree_or_commit]))?;
        parse_ls_tree(&out)
    }
    ...
    pub fn read_blob_bytes(&self, spec: &str) -> Result<Vec<u8>, StoreError> {
        let mut cmd = git_in(self.path(), ["cat-file", "-p", spec]);
    ...
    /// Read one commit's metadata. `sha` is any commit reference the
    /// git CLI accepts.
    pub fn read_commit(&self, sha: &str) -> Result<CommitMeta, StoreError> {
        // A null-terminated separator between the header line group
        // and the message would be safer, but keeping the message at
        // the end and using `splitn(8, '\n')` is sufficient: only the
        // message can contain embedded newlines.
    ...
    /// Resolve `reference` to a full SHA via `git rev-parse --verify`.
