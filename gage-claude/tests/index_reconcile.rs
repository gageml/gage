...
fn reconcile_lifecycle() {
    ...
    let hits = store
        .search("main", 50, gage_claude::index::DEFAULT_SNIPPET_CHARS)
        .unwrap();
    ...
    let hits = store
        .search("zanzibar", 10, gage_claude::index::DEFAULT_SNIPPET_CHARS)
        .unwrap();
...
fn search_on_empty_cache_is_empty() {
    ...
    assert!(
        store
            .search("anything", 10, gage_claude::index::DEFAULT_SNIPPET_CHARS)
            .unwrap()
            .is_empty()
    );
}
...
fn snippet_highlights_use_guillemets() {
    ...
    store.reconcile(LockMode::Wait).unwrap();
    let hits = store
        .search("main", 10, gage_claude::index::DEFAULT_SNIPPET_CHARS)
        .unwrap();
    let snippet = hits
        .iter()
...
fn limit_is_honored() {
    ...
    store.reconcile(LockMode::Wait).unwrap();
    let hits = store
        .search("the", 2, gage_claude::index::DEFAULT_SNIPPET_CHARS)
        .unwrap();
    assert!(hits.len() <= 2);
}
