...
/// Author string for an issue event generated by a tool call:
/// `agent:{name}@{version}` from the calling MCP client's
/// `Implementation` info sent at initialize. Falls back to
/// `agent:unknown` when the peer info is not yet available.
pub fn agent_author(ctx: &RequestContext<RoleServer>) -> String {
    match ctx.peer.peer_info() {
        Some(info) => format!(
            "agent:{}@{}",
