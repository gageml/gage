...
/// Author string for a write generated by a tool call. The author is
/// the authoring call: `{base}?call={toolUseId}`, where the tool-use
/// id from the request's `_meta` ties the written item to the exact
/// transcript entry that wrote it. Requests without a tool-use id
/// (clients that don't send one) fall back to the bare base.
///
/// The base names the caller: the fronted agent's identity
/// (`agent:{scanner}`) when the server serves a single agent
/// invocation, otherwise `agent:{name}@{version}` from the client's
/// initialize info.
