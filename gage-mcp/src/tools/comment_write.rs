...
const MD: &str = include_str!("../../config/tools/CommentWrite.md");
...
async fn handle(params: JsonObject, author: String) -> Result<String, McpError> {
    let text = req_string(&params, "text")?;
    ...
    let mut note = Note::new(target, "comment", NoteValue::from(text), &author);
    note.name = format!("comment.{}", short_uuid(&note.id));

    let conn = open_db().unwrap();
    note::insert(&conn, &note)
        .map_err(|e| McpError::internal_error(format!("insert comment: {e}"), None))?;

    if let Some(scan_id) = &scan_id {
        insert_scan_note(&conn, scan_id, &note.id)
            .map_err(|e| McpError::internal_error(format!("link comment to scan: {e}"), None))?;
    }

    Ok(format!("Added comment {}.", short_uuid(&note.id)))
}
