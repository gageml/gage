...
fn enc_sum(r: &SummaryRecord) -> Vec<u8> {
    rkyv::to_bytes::<Error>(&Sr::from(r)).unwrap().into_vec()
}

fn dec_sum(buf: &[u8]) -> SummaryRecord {
    rkyv::from_bytes::<Sr, Error>(buf).unwrap().into()
}

fn enc_ent(r: &EntriesRecord) -> Vec<u8> {
    rkyv::to_bytes::<Error>(&Er::from(r)).unwrap().into_vec()
}

fn dec_ent(buf: &[u8]) -> EntriesRecord {
    rkyv::from_bytes::<Er, Error>(buf).unwrap().into()
}
