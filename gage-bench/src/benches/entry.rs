//! Per-session entries round-trip. Each iteration writes one file
//! per session containing the full entries vector, then reads them
//! all back; we collect a sample per iteration for mean/stddev
//! reporting.
...
pub fn run(
    ...
    for fmt in formats {
        let fmt_dir = outdir.join("entry").join(fmt.name);
        ...
        for _ in 0..iterations {
            let (_, ser_time) = time(|| {
                for rec in &corpus.entries {
                    let bytes = (fmt.encode_entries)(rec);
            ...
            let (decoded, deser_time) = time(|| {
                let mut out: Vec<EntriesRecord> = Vec::with_capacity(corpus.entries.len());
                for rec in &corpus.entries {
                    let path = fmt_dir.join(format!("{}.bin", rec.session_id));
                    let buf = read_payload(&path, fmt.compress);
                    out.push((fmt.decode_entries)(&buf));
                    progress.inc(1);
                }
                out
            });
            deser_samples.push(duration_ms(deser_time));
            last_records = decoded.iter().map(|r| r.entries.len()).sum();
            drop(decoded);
        }
