...
//! [`Output::Log`] record through the task's output channel. The scan
//! writes records to the task's `logs/records`. Distinct from
//! `println`, which is task output and lands in `logs/out`.
//!
...
use crate::{Level, OUTPUT_TX, Output};
