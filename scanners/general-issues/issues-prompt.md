...
with `name` values `session.finding-summary.<random>`

`SELECT * FROM note WHERE name LIKE 'session.finding-summary.%'`
...
with `name` values `session.finding.<random>`

`SELECT * FROM note WHERE name LIKE 'session.finding.%' AND target = 'session:${session_id}'`
