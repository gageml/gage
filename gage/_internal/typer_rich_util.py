...
# Used to patch typer.core.rich_utils (see gage.__main__)
...
STYLE_NEGATIVE_SWITCH = "bold magenta"
STYLE_METAVAR = "bold italic cyan"
STYLE_METAVAR_SEPARATOR = "not italic cyan"
STYLE_USAGE = "bold"
...
def _print_options_panel(
    *,
    ...
    for param in params:
        ...
        opt_str_cols = (
            [
                highlighter(", ".join(opt_short_strs + opt_long_strs)),
                negative_highlighter(
                    ", ".join(secondary_opt_long_strs + secondary_opt_short_strs)
                ),
            ]
            if any(opt[0] == "-" for opt in (opt_short_strs + opt_long_strs))
            else []
        )
...
def _print_commands_panel(
    *,
    ...
    # regex
    commands_table.add_column(style=STYLE_COMMANDS, no_wrap=True)
...
def rich_format_help(
    *,
    ...
    # Help is logged to console at this point. Exit now to avoid the
    # extra line ending printed by click, who's expecting help to be
    # rendered via the help formatter.
    raise SystemExit(0)


def rich_format_error(e: click.ClickException) -> None:
    ...
    ctx: Union[click.Context, None] = getattr(e, "ctx", None)
    if ctx is not None:
        console.print(highlighter(ctx.get_usage()), style=STYLE_USAGE_COMMAND)
        console.print()

    console.print(highlighter(e.format_message()))
